...
namespace Business.Utils
{
        ...
        where TEntityUpdateDto : IDto, new()
    {
        ...
        public virtual async Task<IDataResult<TEntityGetDto>> UpdateAsync(int id, TEntityUpdateDto input)
        {
            var entity = await BaseEntityRepository.GetAsync(x => x.Id == id);

            if (entity == null)
            {
                return new ErrorDataResult<TEntityGetDto>($"'{id}' id'li {typeof(TEntity).Name} entitysi bulunamad�.");
            }
        ...
        public virtual async Task<IResult> DeleteByIdAsync(int id)
        {
            try
            {
        ...
        public virtual async Task<IDataResult<TEntityGetDto>> GetByIdAsync(int id)
        {
            var entity = await BaseEntityRepository.GetAsync(x => x.Id == id);
